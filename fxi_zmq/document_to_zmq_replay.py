...
class readable_dir(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        if not os.path.isdir(prospective_dir):
            raise argparse.ArgumentError(self, "msgpack directory:{0} is not a valid path".format(prospective_dir))
        if os.access(prospective_dir, os.R_OK):
            setattr(namespace,self.dest,prospective_dir)
        else:
            raise argparse.ArgumentError(self, "msgpack directory:{0} is not a readable dir".format(prospective_dir))
...
class FxiStandardFlyScan(DocumentRouter):
    ...
    def datum_page(self, doc):
        for i, datum_id in enumerate(doc['datum_id']):
            ...
            print(datum_id, point_number, doc["resource"], doc.keys(), doc['datum_kwargs'])
    ...
    def descriptor(self, doc):
        if doc['name'] == 'baseline':
            ...
            file_name = doc["configuration"]["Andor"]["data"]["Andor_hdf5_full_file_name"]
...
async def main():
    global fxi_zmq
    parser = argparse.ArgumentParser(description='FXI-18 Document Stream to ZMQ Stream Transformer', fromfile_prefix_chars="@")
    parser.add_argument('-o', '--output_zmq_publish_port', type=int, default=0, required=True)
    sp = parser.add_subparsers()
    sp_msgpack = sp.add_parser('msgpack', help='Msgpack option')
    sp_msgpack.add_argument('-m', '--msgpack_directory', action=readable_dir, required=True)
